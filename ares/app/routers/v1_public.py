...
async def v1_random_games(  request: Request, 
                            n: int = Query(0, title="Number of random games", gt=0, le=100), 
                            labels: list[str] = Query(default=['base']),
                            debug: bool = False ) -> dict:
...
async def v1_collection_by_id(request: Request, 
                              labels: list[str] = Query(default=['base']),
                              collectionID: int = Path(0, title="Collection ID"), 
                              debug: bool = False) -> dict:
...
async def get_top_rated_collection( request: Request, 
    ...
    topRatedCollection, debug_data = await iris_cli.get_top_rated_collection(limit, debug)

    if debug: return {"debug": debug_data, "data": topRatedCollection}
